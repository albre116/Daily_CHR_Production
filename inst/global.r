...
{k <- nrow(km$centers)
 ...
 if(k==1){out<-rep(1,n)
          min_err<-(data[,1]-km$centers[1,1])^2}else{
            out <- apply(d, 1, which.min)
            min_err<-lapply(1:nrow(d),function(i){
              (data[i,1]-km$centers[out[i],1])^2
            })}
 min_err<-as.numeric(min_err)/length(min_err)
 return(list(out=out,d=d,min_err=min_err))}
...
averages<-function(X,d_index=c(1)){###identify date index in input file
  ...
  for (i in 1:length(index)){
    ...
    DAY[i,]<-apply(Z[tmp,,drop=F],2,mean)
    HEADER[i]<-date[tmp][1]
    VOLUME[i]<-length(tmp)
  }
...
align_day<-function(X,d_index,start="2008-01-01",end="2013-12-01"){###identify date index in input files
  y<-names(X)
  ...
  for (i in 1:length(y)){
    ...
    dat_temp<-as.data.frame(matrix(nrow=row,ncol=col))
    #eval(parse(text=tmpcmd))
    for(j in 1:passes){
      #tmpcmd<-paste(y[i],"[which(HEADER==X$",y[i],"[j,",d_index[i],"]),]=X$",y[i],"[j,]",sep="")
      dat_temp[HEADER==X[[i]][j,1],]=X[[i]][j,]
      #eval(parse(text=tmpcmd))
    }
    names<-colnames(X[[i]])
    #tmpcmd<-paste("names=colnames(X$",y[i],")",sep="")
    #eval(parse(text=tmpcmd))
    colnames(dat_temp)<-names
...
#The overall output is a list of these row lists. Usful for converting data for a java plotter
